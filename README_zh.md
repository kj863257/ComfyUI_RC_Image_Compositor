...
> ✅ 最近更新：全面支持视频（多帧）处理、新增 Photoshop 品质色调匹配节点（4种高级算法）、Lightroom/Camera Raw 预设浏览器（交互式预览）、Blend If 滑块视觉提示升级、曲线编辑器适配 ComfyUI 缩放高清重绘、渐变色标支持拖动定位、新增高性能图案平铺与元数据移植流程，现又加入交互式画布节点，实现所见即所得的前景摆放。

目前插件合集共提供 **36 个专业节点**，覆盖图层合成、图层样式、色彩调整、实用工具和生成类功能。
...
- `RC 色调匹配 | RC Color Match` - Photoshop 级色调匹配，支持 4 种高级算法（LAB 统计、LAB 直方图、MKL 转移、小波混合）、强度控制、亮度保护、去中性化，完整视频批处理支持
